...
class RxRealmWriteSinks: XCTestCase {
    ...
    func testRxAddObject() {
        ...
        XCTAssertTrue(observer.events[0].value.element!.equalTo(to: [Message("1")]))
    }
    ...
    func testRxAddObjects() {
        ...
        XCTAssertEqual(observer.events.count, 1)
        XCTAssertEqual(observer.events[0].time, 0)
        XCTAssertTrue(observer.events[0].value.element!.equalTo(to: [Message("1"), Message("2")]))
    }
    ...
    func testRxAddObjectsInBg() {
        ...
        [Message("1")].toObservable.from()
            .subscribe( realm.rx_add() )
            .addDisposableTo(bag)
        
        delayInBackground(delay: 0.1, closure: {
            // subscribe/write on background thread
            let realm = try! Realm(configuration: conf)
            [Message("2")].toObservable.from()
                .subscribe(realm.rx_add() )
                .addDisposableTo(bag)
        })
        
        // subscribe on current/write on main
        [Message("3")].Observable.from()
        ...
        Observable.from([Message("4")])
            .observeOn( ConcurrentDispatchQueueScheduler(
                queue: DispatchQueue.global(Int(UInt32(QOS_CLASS_BACKGROUND.rawValue)), 0)))
            .subscribe( Realm.rx_add(conf) )
            .addDisposableTo(bag)

        // subscribe on current/write on background
        [[Message("5"), Message("6")]].Observable.from()
            .observeOn( ConcurrentDispatchQueueScheduler(
                queue: DispatchQueue.global(Int(UInt32(QOS_CLASS_BACKGROUND.rawValue)), 0)))
            .subscribe( Realm.rx_add(conf) )
            .addDisposableTo(bag)
